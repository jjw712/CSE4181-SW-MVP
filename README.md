...
LLM은 검색어 보강과 요약에만 사용합니다. 출처 없는 내용은 리포트 근거로 쓰지 않습니다.

## 데이터 소스

| 소스 | 용도 | 상태 |
| --- | --- | --- |
| Naver Search API | 웹, 블로그, 뉴스, 카페 문서 검색 | MVP |
| Naver News API | 국내 뉴스 기반 정책, 시장, 사회 이슈 확인 | MVP |
| Naver DataLab | 키워드별 상대 검색 관심도 확인 | MVP |
| Apple iTunes Search API | iOS 경쟁 앱 후보와 앱 메타데이터 수집 | MVP |
| GDELT | 글로벌 뉴스 기반 PEST 신호 수집 | MVP |
| Hacker News Algolia API | 개발자/스타트업 커뮤니티 반응 확인 | MVP |
| GitHub REST API | 관련 오픈소스, API, 기술 생태계 확인 | MVP |
...
실행 전에 `.env` 파일을 만들고 필요한 값을 설정합니다.

| 변수 | 필수 여부 | 설명 |
| --- | --- | --- |
| `OPENAI_API_KEY` | 필수 | LLM 기반 검색어 생성, 요약, JSON 구조화에 사용 |
| `NAVER_CLIENT_ID` | 필수 | Naver Search API와 Naver DataLab 호출에 사용 |
| `NAVER_CLIENT_SECRET` | 필수 | Naver Search API와 Naver DataLab 호출에 사용 |
...
`.env` 파일은 commit하지 않습니다. 필요한 변수 이름만 `.env.example`에 적습니다.
...
- LLM 출력은 요약과 구조화 용도입니다. 사실 데이터처럼 표시하지 않습니다.
